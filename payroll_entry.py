...
class PayrollEntry(Document):
	...
	def get_salary_component_account(self, salary_component):
		account = frappe.db.get_value("Salary Component Account",
			{"parent": salary_component, "company": self.company}, ["default_account","payable_account"], as_dict=True)

		if not account:
			frappe.throw(_("Please set default account in Salary Component {0}")
				.format(salary_component))
		elif not account["payable_account"]:
			frappe.throw(_("Please set credit account in Salary Component {0}")
				.format(salary_component))
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			salary_components = frappe.db.sql("""select salary_component, amount, parentfield,(select employee from `tabSalary Slip` where `name`=D.parent) Employee,customer
				from `tabSalary Detail` D where parentfield = '%s' and parent in (%s)""" %
				(component_type, ', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=True,debug=True)
	...
	def get_account(self, component_dict = None):
		...
		for item in component_dict:
			...
			inner_dict["customer"] = item["customer"]
	...
	def make_accrual_jv_entry(self):
		...
		if earnings or deductions:
			...
			accounts = []
			payable_amount = 0
			# frappe.throw("earnings============>"+str(earnings))
			for item in earnings:
				...
				accounts.append({
						...
						"cost_center": frappe.db.get_value("Customer",item["customer"],"cost_center") or self.cost_center,
						"customer": (item["customer"] if item["customer"] else ""),
						"party":item["employee"],
						"party_type":"Employee",
